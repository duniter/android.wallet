...
//                        });
    ...
    public static String quantitativeFormatter(Context context, BigInteger amount,String currencyName){
        BigInteger result;
        String plus;
        if(amount.compareTo(new BigInteger("100000000000000"))>=1){
            result = amount.divide(new BigInteger("1000000000000"));
            plus = TERA;
        }else{
            if(amount.compareTo(new BigInteger("100000000000"))>=1){
                result = amount.divide(new BigInteger("1000000000"));
                plus = GIGA;
            }else{
                if(amount.compareTo(new BigInteger("100000000"))>=1){
                    result = amount.divide(new BigInteger("1000000"));
                    plus = MEGA;
                }else{
                    if(amount.compareTo(new BigInteger("100000"))>=1){
                        result = amount.divide(new BigInteger("1000"));
                        plus = KILO;
                    }else{
                        result = amount;
                        plus = "";
